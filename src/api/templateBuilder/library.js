...
export async function getLibraryItem(itemId) {
  const { data } = await axiosClient.get(
    `/template-builder/library/item/${encodeURIComponent(itemId)}`
  );
  return data;
}
...
export async function activateLibraryItem(itemId, payload) {
  const { data } = await axiosClient.post(
    `/template-builder/library/${encodeURIComponent(itemId)}/activate`,
    payload
  );
  return data;
}
...
export async function exportLibrary(industry) {
  const { data } = await axiosClient.get("/template-builder/library/export", {
    params: { industry },
  });
  return data;
}
